...
extern bool FileGetContent(const char* path,ImVector<unsigned char>& bufferOut,bool openInTextMode=false,const char* password=NULL);  // password is used if it's a file inside a zip path when IMGUI_USE_MINIZIP is defined (e.g. path="C://MyDocuments/myzipfile.zip/myzipFile/something.txt")
extern bool FileGetContent(const char* path,ImVector<char>& bufferOut,bool openInTextMode=false,const char* password=NULL);  // password is used if it's a file inside a zip path when IMGUI_USE_MINIZIP is defined (e.g. path="C://MyDocuments/myzipfile.zip/myzipFile/something.txt")
