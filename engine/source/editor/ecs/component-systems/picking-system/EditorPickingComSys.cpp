...
//#define USE_IMGUIZMO_CAM // imguizmo module has its own camera MVP calculation methods, try use it just case you get a different result
...
void longmarch::EditorPickingComSys::ManipulatePickedEntityGizmos(const Entity& picked_entity)
{
    ...
    if (!InputManager::GetInstance()->IsMouseButtonPressed(MOUSE_BUTTON_RIGHT) && !ImGui::IsAnyItemActive() && !
        ImGuizmo::IsUsing())
    {
    ...
    if (camera != picked_entity && trans.Valid())
    {
        ...
        if (m_sceneDockDrawList)
        {
            ...
            ImGuizmo::SetRect((float)prop.m_xpos + (float)viewport_origin.x,
                              (float)prop.m_ypos + (float)viewport_origin.y, (float)viewport_size.x,
        ...
        // Draw guizmo
        if (ImGuizmo::Manipulate(&(cam_view[0][0]), &(cam_proj[0][0]), operation, mode, &(trans_mat[0][0]), nullptr,
