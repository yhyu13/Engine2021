...
longmarch::SceneHierarchyDock::SceneHierarchyDock()
{
...

    {
        ...
        ManageEventSubHandle(queue->Subscribe<SceneHierarchyDock>(this, EngineIOEventType::LOAD_SCENE_BEGIN,
                                                                  &SceneHierarchyDock::_ON_LOAD_SCENE_BEGIN));
    }
}
        ...
        if (ImGui::Button("Add Entity"))
        {
            ...
            if (bShouldShow)
            {
                ...
                m_addEntityPopup = [this, entity_0]()
                {
                    ...
                    if (ImGui::BeginPopupModal("AddEntityTypePopup", NULL,
        ...
        if (ImGui::Button("Remove Entity"))
        {
            ...
            if (bShouldShow)
            {
                ...
                m_addEntityPopup = [this, entity_0]()
                {
                    ...
                    if (ImGui::BeginPopupModal("RemoveEntityTypePopup", NULL,
                                               ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoDecoration |
                                               ImGuiWindowFlags_AlwaysAutoResize))
                    {
                        ...
                        {
                            ImGui::Text(
                                Str("Are you sure to remove: " + nameCom->GetName() + "(" + nameCom->GetUniqueName() +
                                    ")?").c_str());
                            if (ImGui::Button("Yes", ImVec2(80, 0)))
                            {
                                auto queue = EventQueue<EngineEventType>::GetInstance();
                                auto e = MemoryManager::Make_shared<EngineGCRecursiveEvent>(EntityDecorator{
                                    entity_0, GameWorld::GetCurrent()
                                });
                                queue->Publish(e);
...
void longmarch::SceneHierarchyDock::GenerateTreeNode(const Entity& e)
{
    ...
    if (childrenCom.Valid())
    {
        ...
        {
            bool node_open = ImGui::TreeNodeEx(idNameCom->GetUniqueName().c_str(), node_flags,
                                               idNameCom->GetName().c_str());
            if (ImGui::IsItemClicked())
            {
                ...
                if (ImGui::IsMouseDoubleClicked(0))
                {
                    ...
                    */
                    ...
                    auto event = MemoryManager::Make_shared<EditorCameraTeleportToEntityEvent>(EntityDecorator{
                        e, GameWorld::GetCurrent()
                    });
...
void longmarch::SceneHierarchyDock::SetSelectionMaskExclusion(const Entity& e, bool mask,
                                                              bool exclusive_set_others)
{
...
void longmarch::SceneHierarchyDock::SetSelectionMaskExclusion(const std::set<Entity>& es, bool mask,
                                                              bool exclusive_set_others)
{
