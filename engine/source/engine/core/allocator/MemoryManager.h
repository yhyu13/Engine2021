...
namespace longmarch
{
	...
	private:
		...
		inline constexpr static void Delete(T* p) noexcept
		{
			...
			Free(p, sizeof(T));
#else
			//DEBUG_PRINT("Delete : " + Str(typeid(T).name()) + " " + Str(sizeof(T)) + " " + Str(s_AllocatedSize));
			delete p;
#endif // CUSTOM_ALLOCATOR
		}
	...
	private:
		...
		inline static std::atomic<size_t>		s_AllocatedSize = { 0u };
	...
	template<class T>
	struct A4Games_Deleter { // deleter
	...
	*/
	...
	struct Mallocator
	{
		...
		[[nodiscard]] T* allocate(std::size_t n) {
			...
			if (auto p = static_cast<T*>(MemoryManager::Allocate(n * sizeof(T))); p) [[likely]]
			{
