	...
	namespace emilib
	{
		...
		*/
		...
		public:
			...
			Coroutine(std::string_view debug_name, std::function<void(InnerControl& ic)> fun);
		...
		private:
			...
			std::unique_ptr<InnerControl> _inner = { nullptr };
		...
		private:
			inline static std::atomic_uint s_cr_counter = { 0u };
		};
		...
		public:
			...
			std::shared_ptr<Coroutine> start(std::string_view debug_name, std::function<void(InnerControl& ic)> fun);
