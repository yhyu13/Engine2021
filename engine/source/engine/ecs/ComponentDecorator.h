...
namespace longmarch {
	...
	public:
		...
		ComponentDecorator(const EntityDecorator& owner, ComponentType* component)
			:
		...
		inline bool Update(bool throw_on_invalid = true)
		{
			...
			else
			{
				if (throw_on_invalid)
				{
					ENGINE_EXCEPT(str2wstr(Str(m_entity)) + L" fails to update component " + wStr(typeid(ComponentType).name()));
				}
				return false;
			}
		}
