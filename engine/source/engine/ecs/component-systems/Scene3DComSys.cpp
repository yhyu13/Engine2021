    ...
    **************************************************************/
    {
        ...
        const auto& lights = m_parentWorld->GetAllEntityWithType({
            (EntityType)EngineEntityType::DIRECTIONAL_LIGHT,
                ...
                if (sceneDataRef)
                {
                    ...
                    if (!emissive || sceneData.empty())
                    {
                        throw EngineException(
                            _CRT_WIDE(__FILE__), __LINE__, L"Lights need at least one emssive material!");
                    }
                }
            }
        }
    ...
    **************************************************************/
    {
        ...
        struct RenderTransparentObj_CPU_ComparatorLesser
            // used in priority queue that puts objects in greater distances at front
        {
        ...
        std::priority_queue<RenderTransparentObj_CPU, LongMarch_Vector<RenderTransparentObj_CPU>,
                            RenderTransparentObj_CPU_ComparatorLesser> depth_sorted_translucent_obj;
...
void longmarch::Scene3DComSys::PrepareScene(double dt)
{
    ...
    const auto& job = BackEach([dt, this](EntityDecorator e)
    {
        auto scene = GetComponent<Scene3DCom>(e);
        scene->SetShouldDraw(true, true);
        if (scene->IsVisible())
        {
            if (scene->IsTranslucenctRenderType() || scene->IsParticleRenderType())
            [[unlikely]]
            {
                LOCK_GUARD_NC();
                Renderer3D::s_Data.cpuBuffer.RENDERABLE_OBJ_TRANSPARENT.emplace_back(EntityDecorator{
                    e, this->m_parentWorld
                });
            }
            else
            [[likely]]
            {
                LOCK_GUARD_NC();
                Renderer3D::s_Data.cpuBuffer.RENDERABLE_OBJ_OPAQUE.
                                   emplace_back(EntityDecorator{e, this->m_parentWorld});
            }
        }
    });
...
void longmarch::Scene3DComSys::SetVFCullingParam(bool enable, const ViewFrustum& VF,
                                                 const Mat4& WorldSpaceToViewFrustum)
{
