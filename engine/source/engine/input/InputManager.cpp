...
namespace AAAAgames
{
	...
	void InputManager::Update(double deltaTime)
	{
		...
		{
			LOCK_GUARD_NC();
			memcpy(m_previousKeyBoardState, m_currentKeyBoardState, sizeof(m_currentKeyBoardState)); // copy current state to previous state
			memcpy(m_previousMouseButtonState, m_currentMouseButtonState, sizeof(m_currentMouseButtonState)); // copy current state to previous state
	...
	bool InputManager::IsKeyTriggered(int keyCode) const
	{
		...
		else if (std::chrono::duration<double>(m_updateTimeStamp - m_pressKeyBoardTimeStamp[keyCode]).count() <= m_deltaTime
			&& std::chrono::duration<double>(m_updateTimeStamp - m_releaseKeyBoardTimeStamp[keyCode]).count() <= m_deltaTime)
		{
			return true;
		}
		else
	...
	bool InputManager::IsKeyReleased(int keyCode) const
	{
		...
		else if (std::chrono::duration<double>(m_releaseKeyBoardTimeStamp[keyCode] - m_pressKeyBoardTimeStamp[keyCode]).count() <= 0.0
			&& std::chrono::duration<double>(m_updateTimeStamp - m_pressKeyBoardTimeStamp[keyCode]).count() <= m_deltaTime
			&& std::chrono::duration<double>(m_updateTimeStamp - m_releaseKeyBoardTimeStamp[keyCode]).count() <= m_deltaTime)
		{
			return true;
		}
	...
	bool InputManager::IsMouseButtonTriggered(int mouseButton) const
	{
		...
		else if (std::chrono::duration<double>(m_updateTimeStamp - m_pressMouseButtonTimeStamp[mouseButton]).count() <= m_deltaTime
			&& std::chrono::duration<double>(m_updateTimeStamp - m_releaseMouseButtonTimeStamp[mouseButton]).count() <= m_deltaTime)
		{
			return true;
		}
		else
	...
	bool InputManager::IsMouseButtonReleased(int mouseButton) const
	{
		...
		else if (std::chrono::duration<double>(m_releaseMouseButtonTimeStamp[mouseButton] - m_pressMouseButtonTimeStamp[mouseButton]).count() <= 0.0
			&& std::chrono::duration<double>(m_updateTimeStamp - m_pressMouseButtonTimeStamp[mouseButton]).count() <= m_deltaTime
			&& std::chrono::duration<double>(m_updateTimeStamp - m_releaseMouseButtonTimeStamp[mouseButton]).count() <= m_deltaTime)
		{
			return true;
		}
	...
	const float InputManager::GetMousePositionX() const
	{
		LOCK_GUARD_NC();
		return m_cursorPosition.x;
	}

	const float InputManager::GetMousePositionY() const
	{
		LOCK_GUARD_NC();
		return m_cursorPosition.y;
	}
	void InputManager::SetMouseMaxPositions(float x, float y)
	{
		LOCK_GUARD_NC();
	...
	const float InputManager::GetMouseMaxPositionX() const
	{
		LOCK_GUARD_NC();
		return m_cursorMaxPosition.x;
	}
	const float InputManager::GetMouseMaxPositionY() const
	{
		LOCK_GUARD_NC();
		return m_cursorMaxPosition.y;
	}

	const Vec2f InputManager::GetCursorViewPosition() const
	{
		LOCK_GUARD_NC();
