...
namespace longmarch
{
	...
	public:
		...
		const float GetMousePositionX() const;
		const float GetMousePositionY() const;
		void SetMouseMaxPositions(float x, float y);
		const Vec2f GetCursorMaxPositionXY() const;
		const float GetMouseMaxPositionX() const;
		const float GetMouseMaxPositionY() const;

		// mouse cursor coordinate : origin at upper left conrer, +x = right, +y = down. Range from [0,width],[0,height]
		// screen space(aka. view) normalized coordinate : origin at center, +x= right, +y = up. Range from [-aspectWByH,aspectWByH],[-1,1]
		const Vec2f GetCursorViewPosition() const;
