...
namespace AAAAgames
{
	...
	typedef glm::i64vec3 Vec3i64;
	typedef glm::i64vec4 Vec4i64;
	typedef glm::f64vec2 Vec2f64;
	typedef glm::f64vec3 Vec3f64;
	typedef glm::f64vec4 Vec4f64;
	typedef glm::dmat3 Mat3f64;
	typedef glm::dmat4 Mat4f64;

	typedef glm::f32quat Quaternion;
	typedef glm::f64quat Quaternion64;

	typedef glm::vec<4, glm::uint16, glm::packed_highp> Vec4hf_pack;
	typedef glm::vec<2, glm::uint16, glm::packed_highp> Vec2hf_pack;
	...
	*/
	...
	public:
		...
		static Mat4 ProjectionMatrixNegativeOneOne(double fovY_radians, double aspectWbyH, double zNear, double zFar, bool inf = false);

		static Mat4 ProjectionMatrixZeroOne(double fovY_radians, double aspectWbyH, double zNear, double zFar, bool inf = false);

		static Mat4 ReverseZProjectionMatrixZeroOne(double fovY_radians, double aspectWbyH, double zNear, double zFar, bool inf = false);

		static Mat4 OrthogonalProjectionMatrixZeroOne(double left, double right, double buttom, double top, double zNear, double zFar);

		static Mat4 OrthogonalReverseZProjectionMatrixZeroOne(double left, double right, double buttom, double top, double zNear, double zFar);

		struct Frustum
		{
			static ViewFrustum FromProjection(const Mat4& m);

			static void GetCornersAndCentroid(const Mat4& pv, bool reverse_z, ViewFrustumCorners& out_world_ndc_corners, Vec3f& out_world_center);
		};
