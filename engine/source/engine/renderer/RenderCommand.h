...
namespace longmarch
{
    ...
    public:
        ...
        {
            ScopedDebugMarker() = delete;
            explicit ScopedDebugMarker(const char* msg) { s_RendererAPI->PushDebugMarker(msg); }
            ~ScopedDebugMarker() { s_RendererAPI->PopDebugMarker(); }
        };

#define GPU_DEBUG_SCOPE(x) RenderCommand::ScopedDebugMarker gpu_scope##__LINE__{x}
#define GPU_NAMED_SCOPE(x) RenderCommand::ScopedDebugMarker gpu_scope##__LINE__{#x}
        ...
        inline static void MultiDrawTriangleIndexedIndirect(const std::shared_ptr<VertexArray>& vertexArray,
        ...
        inline static void TransferAllBit(uint32_t src, uint32_t src_w, uint32_t src_h, uint32_t dest, uint32_t dest_w,
                                          uint32_t dest_h)
        {
        ...
        inline static void TransferColorBit(uint32_t src, uint32_t src_w, uint32_t src_h, uint32_t dest,
        ...
        //! Linear blend color bit
        inline static void TransferColorBit(uint32_t src, uint32_t src_tex, uint32_t src_w, uint32_t src_h,
                                            uint32_t dest, uint32_t dest_tex, uint32_t dest_w, uint32_t dest_h)
        {
            s_RendererAPI->TransferColorBit(src, src_tex, src_w, src_h, dest, dest_tex, dest_w, dest_h);
        }

        //! Nearest blend depth bit
        inline static void TransferDepthBit(uint32_t src, uint32_t src_w, uint32_t src_h, uint32_t dest,
                                            uint32_t dest_w, uint32_t dest_h)
        {
            s_RendererAPI->TransferDepthBit(src, src_w, src_h, dest, dest_w, dest_h);
        }
        ...
        inline static void TransferStencilBit(uint32_t src, uint32_t src_w, uint32_t src_h, uint32_t dest,
                                              uint32_t dest_w, uint32_t dest_h)
        {
            s_RendererAPI->TransferStencilBit(src, src_w, src_h, dest, dest_w, dest_h);
        }
