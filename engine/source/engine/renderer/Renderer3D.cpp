	...
	**************************************************************/
	{
		...
		**************************************************************/
		...
		s_Data.ShaderMap["GaussianBlur_AO"] = Shader::Create("$shader:dynmaic_ao_guassian_blur.vert", "$shader:dynmaic_ao_guassian_blur.frag");
		...
		for (auto it = s_Data.ShaderMap.begin(); it != s_Data.ShaderMap.end(); ++it)
		{
			...
			it->second->SetInt("u_FragTexture3", s_Data.fragTexture_2_slot);
		...
		case RENDER_PIPE::CLUSTER:
			s_Data.ShaderMap["Default"] = s_Data.ShaderMap["ClusterShader"];
			break;
		case RENDER_PIPE::DEFERRED:
			s_Data.ShaderMap["Default"] = s_Data.ShaderMap["GBufferShader"];
			break;
		case RENDER_PIPE::FORWARD:
			s_Data.ShaderMap["Default"] = s_Data.ShaderMap["ForwardShader"];
			break;
		}
		s_Data.CurrentShader = s_Data.ShaderMap["Default"];
		...
		**************************************************************/
		{
			for (uint32_t i(3); i < 51; i += 2)
			{
				{
					auto [offsets, weights] = DistributionMath::Gaussian1DHalf(i, 0, i * 0.5f);
				...
				{
					auto [offsets, weights] = DistributionMath::Gaussian1DHalfBilinear(i, 0, i * 0.5f);
					auto weight = ShaderStorageBuffer::Create(&weights[0], weights.X() * sizeof(float));
					auto offset = ShaderStorageBuffer::Create(&offsets[0], offsets.X() * sizeof(float));
		...
		// particle VAO
		{
			...
			s_Data.gpuBuffer.particleInstBO = VertexBuffer::Create(nullptr, MAX_PARTICLE_INSTANCES * PARTICLE_INSTANCED_DATA_LENGTH * sizeof(GLfloat)); // TODO size is not zero should be size of instanced data (number of floats * number of max particles)
...
{
	{
		...
		RenderCommand::SetViewport(0, 0, s_Data.resolution.x, s_Data.resolution.y);
		...
		// Prepare buffers
		{
			...
			{
				s_Data.window_size_changed_this_frame = true;
				s_Data.gpuBuffer.FinalFrameBuffer = FrameBuffer::Create(s_Data.resolution.x, s_Data.resolution.y, FrameBuffer::BUFFER_FORMAT::Float16);
			}
		...
		{
			ENG_TIME("Scene phase: Populate Data");
		...
		// Begin geomtry passes
		{
			...
			case RENDER_PIPE::CLUSTER:
				Renderer3D::_BeginClusterBuildGrid(camera);
				Renderer3D::_BeginForwardGeomtryPass(camera, clear_color, s_Data.gpuBuffer.CurrentFrameBuffer);
				break;
			case RENDER_PIPE::DEFERRED:
				Renderer3D::_BeginDeferredGeomtryPass(camera, clear_color, s_Data.gpuBuffer.CurrentGBuffer);
				break;
			case RENDER_PIPE::FORWARD:
				Renderer3D::_BeginForwardGeomtryPass(camera, clear_color, s_Data.gpuBuffer.CurrentFrameBuffer);
				break;
	...
	}
	{
		...
		// Rendering
		{
			ENG_TIME("Scene phase: LOOPING");
			f_setRenderShaderName("Default");
			f_setVFCullingParam(false, camera->GetViewFrustumInViewSpace(), camera->GetViewMatrix());
			f_setDistanceCullingParam(false, Vec3f(), 0, 0);
			f_render();
		}
		{
			ENG_TIME("Scene phase: BATCH RENDER");
			CommitBatchRendering();
		}
	}
}
...
void longmarch::Renderer3D::BeginPostProcessing()
{
	...
	if (s_Data.enable_deferredShading)
	{
		...
		Renderer3D::_BeginMotionBlurPass(s_Data.gpuBuffer.CurrentFrameBuffer, s_Data.gpuBuffer.FrameBuffer_2);
		...
		case RENDER_MODE::MULTIDRAW:
		{
			...
			if (Mat->textures.has_ao())
			{
				...
				if (offset == 0) // only emplace new unique texture
				{
					offset = texture_offset++;
					textureIds.emplace_back(s_Data.fragTexture_empty_slot + offset);
					textures_to_bind.emplace_back(s_Data.fragTexture_empty_slot + offset, Material::MAT_TEXTURE_TYPE::AO);
				}
