...
#define DEBUG_PRINT_SCENE_NODE 0

/**
 * It recursively traverses the scene graph and loads the mesh data into a `Scene3DNode` object
 * 
 * @param sceneNodeName The name of the scene node to load.
 * 
 * @return A shared pointer to a Scene3DNode
 */
void longmarch::Scene3DManager::LoadSceneNodeFromAssimp(const std::string& sceneNodeName)
{
    ...
    /* Loading the scene data into the scene node. */
    RecurseLoad(sceneData, sceneNodeName, path.remove_filename(), sceneNode->GetScene(), sceneNode->GetRoot(),
                AssimpHelper::Glm2Assimp(Mat4(1.0f)), 0);
    ...
    **************************************************************/
    ...
    if (!rm->HasLoadFromFileFunc())
    {
        ...
        {
            auto sceneNode = ResourceManager<AssimpSceneObject>::GetInstance()->LoadFromFile(path.string(), path)->
                Get();
            auto name = ResourceManager<AssimpSceneObject>::GetInstance()->GetName(sceneNode);
            Scene3DNode sceneData{name};
            RecurseLoad(sceneData, name, fs::path(path).remove_filename(), sceneNode->GetScene(), sceneNode->GetRoot(),
                        AssimpHelper::Glm2Assimp(Mat4(1.0f)), 0);
...
/**
 * Load a scene node from the resource manager and set it as the scene data for the entity
 * 
 * @param rootEntity The entity that will be the root of the scene.
 * @param sceneNodeName The name of the scene node to load.
 */
...
void longmarch::Scene3DManager::RecurseLoad(Scene3DNode& sceneData, const std::string& sceneName,
                                            const fs::path& sceneDir, const aiScene* aiscene, const aiNode* node,
                                            const aiMatrix4x4& parentTr, unsigned int level)
{
    ...
    for (unsigned int i = 0; i < node->mNumMeshes; ++i)
    {
...

        {
            auto material = MemoryManager::Make_shared<Material>();
            {
                ...
                else if (aiColor4D baseColorFactor; AI_SUCCESS == mtl->Get(
                ...
                else if (ai_real metallicFactor; AI_SUCCESS == mtl->Get(
                ...
                else if (ai_real roughnessFactor; AI_SUCCESS == mtl->Get(
            ...
            auto meshdata = MemoryManager::Make_shared<MeshData>();
            {
                ...
                for (unsigned int t = 0; t < aimesh->mNumVertices; ++t)
                {
                    ...
                    aiVector3D aitan = aimesh->HasTangentsAndBitangents()
                    ...
                    meshdata->vertices.emplace_back(
                        Geommath::PackVec4ToHVec4(Vec4f(aipnt.x * scale, aipnt.y * scale, aipnt.z * scale, s)),
                        Geommath::PackVec2ToHVec2(Vec2f(Geommath::CartesianToSpherical(N).yz)),
                        Geommath::PackVec2ToHVec2(Vec2f(aitex.x, aitex.y)),
                        Geommath::PackVec2ToHVec2(Vec2f(Geommath::CartesianToSpherical(T).yz)));
#endif
                ...
                for (unsigned int i = 0; i < aimesh->mNumFaces; ++i)
                {
                    ...
                    ENGINE_EXCEPT_IF(aiface->mNumIndices != 3,
                                     L"A Triangle mesh " + wStr(node->mName.data) + L" has " + wStr(aiface->mNumIndices)
                                     + L" indices instead of 3!");
