...
{
    {
        ...
        ManageEventSubHandle(queue->Subscribe<NPCPathFindingControllerComSys>(
            this, CS560EventType::GEN_RND_PATH, &NPCPathFindingControllerComSys::_ON_GEN_RND_PATH));
        ManageEventSubHandle(queue->Subscribe<NPCPathFindingControllerComSys>(
            this, CS560EventType::SET_PATHER_CONTROL, &NPCPathFindingControllerComSys::_ON_SET_PATHER_CONTROL));
        ManageEventSubHandle(queue->Subscribe<NPCPathFindingControllerComSys>(
            this, CS560EventType::SET_PATHER_MISC, &NPCPathFindingControllerComSys::_ON_SET_PATHER_MISC));
        ManageEventSubHandle(queue->Subscribe<NPCPathFindingControllerComSys>(
            this, CS560EventType::SET_PATHER_SMOOTH_SETTING,
            &NPCPathFindingControllerComSys::_ON_SET_PATHER_SMOOTH_SETTING));
    }
}
...
void longmarch::NPCPathFindingControllerComSys::_ON_GEN_RND_PATH(EventQueue<CS560EventType>::EventPtr e)
{
    ...
    if (m_parentWorld == event->m_entity.GetWorld())
    {
        ...
        }
        {
            ...
            auto smooth_setting = pathfinding::SmoothingSetting{
                .mid_points_coarse_grain_stride = 0.2f,
            ...
            PathMoverData mover{
                .original_pathLUT = chosen_path, .smooth_pathLUT = smoothed_path,
                .smooth_path_arc_lengthLUT = pathfinding::GetArcLengthLUT(smoothed_path), .path_agent = event->m_entity
            };
            ...
            if (auto trans = data.path_agent.GetComponent<Transform3DCom>(); trans.Valid())
            {
                ...
                trans->SetGlobalRot(Geommath::FromVectorPair(Geommath::WorldFront, p1 - p0) * Quaternion(
                    Geommath::RotationMat(Geommath::ROT_AXIS::Z, PI) * Geommath::RotationMat(
                        Geommath::ROT_AXIS::X, PI * 0.5f)));
            }
...
void longmarch::NPCPathFindingControllerComSys::Update(double dt)
{
    ...
    ForEachUser(
        [this, dt](const EntityDecorator& e)
        {
            if (auto it = m_PatherLUT.find(e); it != m_PatherLUT.end())
            {
                ...
                if (data.current_speed <= 0.01f)
                {
                    ...
                    {
                        anima->SetCurrentAnimation(Animation3DCom::AnimationSetting{
                            .name = "Idle", .playBackSpeed = 1.f, .refresh = false, .looping = true, .pause = false
                        });
                    }
                }
                else
                {
                    if (auto anima = data.path_agent.GetComponent<Animation3DCom>(); anima.Valid())
                    {
                        ...
                        anima->SetCurrentAnimation(Animation3DCom::AnimationSetting{
                            .name = "C_Walk_IP", .playBackSpeed = play_back_speed, .refresh = false, .looping = true,
                            .pause = false
                        });
                    }
                }
                ...
                {
                    auto t0 = t1 - 1;
                    auto factor = (data.current_distance - data.smooth_path_arc_lengthLUT[t0]) / (data.
                        smooth_path_arc_lengthLUT[t1] - data.smooth_path_arc_lengthLUT[t0]);
